...
- **`agentTest.py`** — Interactive REPL: you type what you want; the customer agent uses tools (e.g. `get_products`) against your `.env` database; messages are saved for multi-turn memory. Needs `GCP_PROJECT_ID` + ADC.
- **`agent_test.py`** — Calls `run_turn` once against Gemini; requires `GCP_PROJECT_ID` + ADC. Exits with skip message if GCP project is missing.
